...
public class VoteRatioFeatureScorerTest extends BaseEgaAnswerScorerTests {
  VoteRatioFeatureScorerTest GIVEN = this, WHEN = this, THEN = this, OR = this, AND = this;
  ...
  private void reference_feature_score_pair_produced() {
    ...
    long upVotesCount = 10;
    long downVotesCount = 7;

    double score = new VoteRatioFeatureScorer().transformScore(VotesRatioUtil.calculateVoteRatio(upVotesCount,
        downVotesCount, AnswerScorerConstants.VOTE_RATIO_FACTOR_CANDIDATE_ANSWER));

    referenceFeatureScorePair = new FeatureScorePair(FeatureScorerEnums.VOTE_RATIO_FEATURE_SCORER.toString(), score);
  }

  private void feature_score_pair_produced() throws AnswerScorerException {
    // TODO Auto-generated method stub
    EgaMetaDataScorerComponent metaDataScorerComponent = new VoteRatioFeatureScorer();
    metaDataScorerComponent.generateFeatureScore(candidateAnswers[0]);
    featureScorePair = new FeatureScorePair(FeatureScorerEnums.VOTE_RATIO_FEATURE_SCORER.toString(),
        candidateAnswers[0].getFeatureValue(FeatureScorerEnums.VOTE_RATIO_FEATURE_SCORER.toString()));
  }
