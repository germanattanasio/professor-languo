...
public class NaiveTrigramQueryGeneratorObsolete {

  NaiveTrigramQueryGeneratorObsolete GIVEN = this, AND = this, WHEN = this, THEN = this;

  private Question queryQuestion = null;
  private Query naiveTrigramQuery = null;

  private final String question = "The quick brown fox jumped over the lazy dogs";

  @Test public void test_trigram_query_is_generated_correctly() throws IOException, SearchException {
  ...
  private void trigram_query_is_generated() throws SearchException {
    if (naiveTrigramQuery == null) {
      naiveTrigramQuery = QueryComponentFactory
          .generateQueryComponent(PrimarySearchConstants.NAIVE_TRIGRAM_QUERY_GENERATOR).generateQuery(queryQuestion);
    }
  }
  ...
  private void test_that_generated_trigram_query_match_the_referenced_query() throws IOException {
    ...
    naiveTrigramQuery.extractTerms(queryTerms);
    List<String> genTermsText = new ArrayList<String>();
    for (Term term : queryTerms)
      genTermsText.add(term.text());
    List<String> referTermsText = AnalyzerUtils.collectTokens(new NgramAnalyzer(3), question);
