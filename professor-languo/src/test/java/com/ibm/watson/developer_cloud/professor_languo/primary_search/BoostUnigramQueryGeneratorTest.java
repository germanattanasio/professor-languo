...
public class BoostUnigramQueryGeneratorTest {
  BoostUnigramQueryGeneratorTest GIVEN = this, AND = this, WHEN = this, THEN = this;

  private Question queryQuestion = null;
  private Query standardQuery = null;

  private final String question = "The quick brown fox jumped over the lazy dogs";

  @Test public void test_standard_query_is_generated_correctly()
      throws IOException, SearchException, IngestionException {
    GIVEN.query_question_is_built();
    WHEN.standard_query_is_generated();
    THEN.test_that_generated_standard_query_match_the_referenced_query();
  }

  private void standard_query_is_generated() throws SearchException, IngestionException {
    if (standardQuery == null) {
      SingletonAnalyzer.generateAnalyzer(PrimarySearchConstants.ENGLISH_ANALYZER);
      standardQuery = QueryComponentFactory.generateQueryComponent(PrimarySearchConstants.BOOST_UNIGRAM_QUERY_GENERATOR)
          .generateQuery(queryQuestion);
    }
  }

  private void test_that_generated_standard_query_match_the_referenced_query() throws IOException {
    Set<Term> queryTerms = new HashSet<Term>();
    standardQuery.extractTerms(queryTerms);
